...
struct snd_vars {
    uint32_t nxt; /* 次に送信するセグメントのシーケンス番号*/
    uint32_t una; /* まだ確認応答を受け取っていない最小のシーケンス番号*/
    uint16_t wnd; /* 対向の受信ウィンドウサイズ */
    uint16_t up;
    uint32_t wl1; 
    uint32_t wl2;
};
...
struct rcv_vars {
    uint32_t nxt; /* 次に受信するセグメントに期待するシーケンス番号 */
    uint16_t wnd; /* 自身の受信ウィンドウサイズ */
    uint16_t up;
};
...
struct tcp_pcb {
    ...
    struct rcv_vars rcv;
    uint32_t irs; /* 初期受信シーケンス番号(相手の初期送信シーケンス番号) */
    uint16_t mss;
                    ...
                    ip_endp_t local, ip_endp_t remote)
{
    ...
    if (!pcb || pcb->state == TCP_STATE_CLOSED) {
        ...
        if (!TCP_FLG_ISSET(flags, TCP_FLG_ACK)) {
            ...
            tcp_output_segment(0, seg->ack, TCP_FLG_RST, 0, NULL, 0, local, remote);
        }
        return;
    }
    switch (pcb->state) {
...
tcp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    if (src.addr == IP_ADDR_BROADCAST || src.addr == iface->broadcast || 
        ...
        errorf("only support unicast, src=%s, dst=%s", ep1, ep2);
        return;
    }
        ...
        if (!local.port) {
            for (p = IP_ENDP_DYNAMIC_PORT_MIN; p <= IP_ENDP_DYNAMIC_PORT_MAX; p++) {
                ...
                if (!tcp_pcb_select(local, remote)) {
                    debugf("dinamic assign local port, port=%u", ntoh16(local.port));
                    break;
                }
            }
