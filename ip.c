...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if ((offset & IP_HDR_FLAG_MF) || (offset & IP_HDR_OFFSET_MASK)) {
        errorf("fragmented does not supported");
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    debugf("%s => %s, protocol=%u, len=%zu", addr1, addr2, protocol, len);
    if (src == IP_ADDR_ANY) {
    ...
    if ((dst & iface->netmask) != (iface->unicast & iface->netmask) && dst != IP_ADDR_BROADCAST) {
        errorf("not readched, dst=%s", addr2);
        return -1;
    }
